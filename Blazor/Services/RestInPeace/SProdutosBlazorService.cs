...
namespace Frontend.Services {
	public class SProdutosBlazorService {
		HttpClient httpClient;

		public SProdutosBlazorService(HttpClient client) {
			httpClient = client;
		}
		...
		public async Task<List<EProduct>> GetAll(string companyID, int listCount=-1, int pageNumber=0, string orderBy="id desc")  {
			...
			HttpRequestMessage req = new HttpRequestMessage(HttpMethod.Get, $"api/v1/SProdutos/GetAll/{companyID}?listCount={listCount}&pageNumber={pageNumber}&orderBy={orderBy}");
